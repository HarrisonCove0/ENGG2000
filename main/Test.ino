/*
  Spins the motor for 3 seconds AND turns the laser (LED_BUILTIN,
  pin 13) on for 3 seconds, whenever the IR beacon sensor detects a
  signal (goes from idle to BEACON DETECTED).
...
void setup() {
  ...
  Serial.println(F("Sensor-triggered motor + laser test starting..."));
}
...
void loop() {
  ...
  if (state == LOW && lastState == HIGH) {
    ...
    digitalWrite(LED_BUILTIN, HIGH);   //Laser on
    delay(3000);
    //Sets the power to 0%
    analogWrite(PIN_EN, 0);
    digitalWrite(LED_BUILTIN, LOW);    //Laser off
